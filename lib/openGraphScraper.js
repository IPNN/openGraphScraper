 ...
 * @param options {object} - options that the user has set
 */
const extractContent = async (body, options) => {
  let result;

  try {
    result = await articleParser.extract(body);
  } catch (e) {
  ...
  if (!result) {
    console.warn(`Could not parse content from ${options.url}`)
  }

  return result ? stripHtml(result.content) : null;
};
 ...
 */
const extractMetaTags = async (body, options) => {
  let ogObject = {};
  ...
  ogObject.content = await extractContent(body, options);
...
const requestAndResultsFormatter = async (options) => {
  ...
  return request.get(requestUrl, options)
    .then(async (response) => {
      ...
      const ogObject = await extractMetaTags(formatBody, options)
...
const setOptionsAndReturnOpenGraphResults = async (options) => {
  ...
  if (options.html) {
    ...
    const ogObject = await extractMetaTags(options.html, options);
